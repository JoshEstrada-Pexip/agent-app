...
## 16. Consult and conference: one agent owns the video (2026-09-10)

**Decision (Josh).** v1 keeps video between the original agent and the
customer. Any additional agent — a consult target, or an agent conferenced
in — is audio only: their widget never joins the VMR, so nothing is
transmitted and nothing is received for them. Video for consults and
conferences (Pexip breakout rooms) is v2.

**What the captures showed.** Reading the S3 consult runs settled two
things that had been guesswork:
- The customer's `held` flag is NEVER set — not on a plain hold, not on a
  consult. Genesys parks the customer with `confined`.
- The captures contain the state a conference presents: the customer's
  `confined` drops to false while BOTH agents are still connected and the
  consult markers (`consultParticipantId`, `consultInitiator`) are still on
  the initiating agent.

**Two changes, both narrow.**
1. *Join guard.* The connect trigger now also requires that I am the only
   connected agent (`soleAgent`). A second agent never joins the room; when
   the first agent leaves on a transfer the condition flips and the
   remaining agent joins as before. `getMyCallState` returns `soleAgent`
   separately from `active` on purpose: a conferenced call IS active for
   both agents, so a reconnect resync must not tear it down.
2. *Consult hold override, one added condition.* It now also requires the
   customer to be `confined`. Every captured consult has the customer
   confined, so consult behaviour is unchanged; a conference has the
   customer un-confined, so the original agent keeps video instead of being
   held by a stale `consultInitiator` attribute.

**Hold and transfer are untouched.** On a plain hold `isConsulting` is
false and the agent's own `held` drives everything, exactly as before. The
transfer path is unchanged because by the time the receiving agent's
connect condition fires, the previous agent has already left.

**Resulting behaviour**
| situation | original agent | second agent | customer |
|---|---|---|---|
| 1:1 call | video | — | video |
| hold | dark | — | video, sees agent dark |
| consult | dark (customer confined) | audio only, never in the VMR | video, sees agent dark |
| conference | video | audio only, never in the VMR | video |
| transfer completed | leaves | video once alone | video |

**Second agent's UI:** an "Audio only" pane saying another agent has the
video, instead of the misleading "No active call".

**Validation.** Unit tests for the new conference shape and the join guard,
plus the existing consult tests with fixtures corrected to match the
captures (the customer is confined during a consult). Live re-run of S3.1,
S3.2, S3.3, S4.5 and S4.6 still outstanding — it needs the second lab agent
signed in.
